...
// JWTAuthHeaderOnlyRest 仅从Header读取 x-token 的鉴权（用户端REST风格返回HTTP状态码）
func JWTAuthHeaderOnlyRest() gin.HandlerFunc {
	return func(c *gin.Context) {
...
func JWTAuthHeaderOnly() gin.HandlerFunc {
	return func(c *gin.Context) {
		token := c.Request.Header.Get("x-token")
		if token == "" {
			response.NoAuth("未登录或非法访问，请登录", c)
