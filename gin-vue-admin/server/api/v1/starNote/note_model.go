...
// AgentChat 用户端 AI Agent 一句话指令
// @Tags UserNote
// @Summary 用户端 AI Agent 一句话指令
// @Security ApiKeyAuth
// @Accept application/json
// @Produce application/json
// @Param data body starNoteReq.AgentReq true "指令与笔记上下文"
// @Success 200 {object} response.Response{data=object,msg=string} "获取成功"
// @Router /unote/agent [post]
func (evtApi *NoteModelApi) AgentChat(c *gin.Context) {
	ctx := c.Request.Context()

	var req starNoteReq.AgentReq
	if err := c.ShouldBindJSON(&req); err != nil {
		c.AbortWithStatusJSON(http.StatusBadRequest, gin.H{
			"code":    http.StatusBadRequest,
			"data":    gin.H{},
	...
	result, err := aiProviderService.RunAgent(ctx, req.Instruction, req.Notes)
	if err != nil {
		global.GVA_LOG.Error("AI Agent 执行失败!", zap.Error(err))
		c.AbortWithStatusJSON(http.StatusInternalServerError, gin.H{
			"code":    http.StatusInternalServerError,
			"data":    gin.H{},
			"message": err.Error(),
