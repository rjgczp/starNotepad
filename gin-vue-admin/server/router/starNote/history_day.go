...
func (s *HistoryDayRouter) InitHistoryDayRouter(Router *gin.RouterGroup, PublicRouter *gin.RouterGroup) {
	...
	}
	{
		...
		hdRouterWithoutAuth.GET("getHistoryDayToday", hdApi.GetHistoryDayToday)   // 获取今日历史上的今天（无需鉴权）
		hdRouterWithoutAuth.GET("getHistoryDayFuture", hdApi.GetHistoryDayFuture) // 获取未来50天历史数据（无需鉴权）
	}
}
