...
func Routers() *gin.Engine {
	...
	// 当配置了路由前缀时，同时在前缀路径下提供静态文件服务（管理端前端会拼接前缀访问）
	if global.GVA_CONFIG.System.RouterPrefix != "" {
		Router.StaticFS(global.GVA_CONFIG.System.RouterPrefix+"/"+global.GVA_CONFIG.Local.StorePath, justFilesFilesystem{http.Dir(global.GVA_CONFIG.Local.StorePath)})
	}
	// 跨域，如需跨域可以打开下面的注释
	...
	// 自定义 Swagger 页面（带分组过滤功能）
	Router.GET(global.GVA_CONFIG.System.RouterPrefix+"/docs", func(c *gin.Context) {
		c.File("static/swagger-custom.html")
	})

	// 提供拆分后的 Swagger JSON 文件（使用独立路径避免与 swagger/*any 冲突）
	Router.GET(global.GVA_CONFIG.System.RouterPrefix+"/docs/swagger-user.json", func(c *gin.Context) {
		c.File("docs/swagger-user.json")
	})
	Router.GET(global.GVA_CONFIG.System.RouterPrefix+"/docs/swagger-admin.json", func(c *gin.Context) {
		c.File("docs/swagger-admin.json")
	})
