...
func (s *autoCodeTemplate) Create(ctx context.Context, info request.AutoCode) error {
	...
	if !info.OnlyTemplate {
		const defaultAdminAuthorityID uint = 888
		err = global.GVA_DB.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
			if len(history.ApiIDs) > 0 {
				var apis []model.SysApi
				if err := tx.Find(&apis, "id in (?)", history.ApiIDs).Error; err != nil {
					return err
				}
				authorityIdStr := strconv.Itoa(int(defaultAdminAuthorityID))
				rules := make([][]string, 0, len(apis))
				for _, a := range apis {
					rules = append(rules, []string{authorityIdStr, a.Path, a.Method})
				}
				if len(rules) > 0 {
			...
			if history.MenuID != 0 {
				menuIdStr := strconv.Itoa(int(history.MenuID))
				var exist model.SysAuthorityMenu
				err := tx.Where("sys_base_menu_id = ? AND sys_authority_authority_id = ?", menuIdStr, strconv.Itoa(int(defaultAdminAuthorityID))).First(&exist).Error
				if errors.Is(err, gorm.ErrRecordNotFound) {
					if err := tx.Create(&model.SysAuthorityMenu{MenuId: menuIdStr, AuthorityId: strconv.Itoa(int(defaultAdminAuthorityID))}).Error; err != nil {
						return err
					}
				} else if err != nil {
					return err
				...
				if info.AutoCreateBtnAuth {
					...
					if err := tx.Where("sys_base_menu_id = ?", history.MenuID).Find(&btns).Error; err != nil {
						return err
					}
					if len(btns) > 0 {
						selected := make([]uint, 0, len(btns))
						for _, b := range btns {
							selected = append(selected, b.ID)
						}
						if err := AuthorityBtnServiceApp.SetAuthorityBtn(request.SysAuthorityBtnReq{MenuID: history.MenuID, AuthorityId: defaultAdminAuthorityID, Selected: selected}); err != nil {
							return err
						}
					}
				}
