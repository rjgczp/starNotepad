...
func (userService *UserService) ResetPasswordByUsername(username, oldPassword, newPassword string) (err error) {
	if global.GVA_DB == nil {
		return errors.New("数据库未初始化")
	}
	var user system.SysUser
	if err = global.GVA_DB.Select("id, password").Where("username = ?", username).First(&user).Error; err != nil {
	...
	return global.GVA_DB.Model(&user).Update("password", utils.BcryptHash(newPassword)).Error
}
