    ...
    >
      <template #prefix>
        <span v-if="value" class="inline-flex items-center">
          <i v-if="isIconfont(value)" :class="normalizeIconfontClass(value)" />
          <el-icon v-else>
            <component :is="value" />
          </el-icon>
        </span>
      ...
      <el-option
        ...
        :value="item.key"
      >
        <span class="gva-icon" style="padding: 3px 0 0">
          <i v-if="isIconfont(item.label)" :class="normalizeIconfontClass(item.label)" />
          <el-icon v-else>
...
<script setup>
  ...
  import { iconfontPrefix, iconfontOptions } from '@/core/iconfontRegistry'
  ...
  options.unshift(...iconfontOptions)

  const isIconfont = (val) => {
    if (!val) return false
    return typeof val === 'string' && (val.includes(` ${iconfontPrefix} `) || val.startsWith(`${iconfontPrefix} `) || val.startsWith('icon-'))
  }
  ...
  const normalizeIconfontClass = (val) => {
    ...
    // Support storing short names, e.g. 'coffee' -> 'icon-coffee'
