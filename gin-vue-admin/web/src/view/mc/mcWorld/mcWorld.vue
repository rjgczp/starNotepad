  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column sortable align="left" label="日期" prop="CreatedAt" width="180">
            ...
            <el-table-column sortable align="left" label="世界名称" prop="name" width="120" />

            <el-table-column sortable align="left" label="唯一标识" prop="slug" width="120" />
   ...
   <template #default="scope">
      [富文本内容]
            ...
            <el-table-column align="left" label="MC版本" prop="mcVersion" width="120" />

            <el-table-column align="left" label="平台" prop="platform" width="120" />

            <el-table-column sortable align="left" label="路径" prop="worldPath" width="120" />

            <el-table-column label="地图链接" prop="mapUrl" width="200">
   <template #default="scope">
      [富文本内容]
   </template>
</el-table-column>
            <el-table-column align="left" label="渲染状态" prop="renderStatus" width="120" />

            <el-table-column label="错误信息" prop="renderError" width="200">
   <template #default="scope">
      [富文本内容]
   </template>
</el-table-column>
            <el-table-column align="left" label="最后渲染时间" prop="lastRenderAt" width="180">
   <template #default="scope">{{ formatDate(scope.row.lastRenderAt) }}</template>
</el-table-column>
    ...
    <template #default="scope">{{ formatBoolean(scope.row.isPublic) }}</template>
        ...
        <el-table-column align="left" label="操作" fixed="right" :min-width="appStore.operateMinWith">
            ...
            <el-button  type="primary" link icon="edit" class="table-button" @click="updateMcWorldFunc(scope.row)">编辑</el-button>
    ...
    <RichView v-model="detailForm.description" />
                    ...
                    <el-descriptions-item label="MC版本">
    {{ detailForm.mcVersion }}
</el-descriptions-item>
                    <el-descriptions-item label="平台">
    {{ detailForm.platform }}
</el-descriptions-item>
                    <el-descriptions-item label="路径">
    {{ detailForm.worldPath }}
</el-descriptions-item>
                    <el-descriptions-item label="地图链接">
    <RichView v-model="detailForm.mapUrl" />
</el-descriptions-item>
                    <el-descriptions-item label="渲染状态">
    {{ detailForm.renderStatus }}
</el-descriptions-item>
                    <el-descriptions-item label="错误信息">
    <RichView v-model="detailForm.renderError" />
</el-descriptions-item>
                    <el-descriptions-item label="最后渲染时间">
    {{ detailForm.lastRenderAt }}
</el-descriptions-item>
...
const getTableData = async() => {
  const table = await getMcWorldList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteMcWorldByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateMcWorldFunc = async(row) => {
    const res = await findMcWorld({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteMcWorldFunc = async (row) => {
    const res = await deleteMcWorld({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
