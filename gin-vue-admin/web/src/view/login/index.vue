  ...
  <div id="userLayout" class="w-full h-full relative">
    ...
    >
      <div class="md:w-3/5 w-10/12 h-full flex items-center justify-evenly">
        ...
        >
          <div>
            <div class="flex items-center justify-center" @click="handleLogoClick">
              <Logo :size="6" />
            </div>
            <div class="mb-9">
            ...
            >
              ...
              <el-form-item v-if="showResetPasswordAction" class="mb-2">
                <el-button
                  class="h-11 w-full"
                  plain
                  size="large"
...
<script setup>
  import { captcha, resetPasswordByUsername } from '@/api/user'
  ...
  const showResetPasswordAction = ref(false)
  const handleLogoClick = () => {
    if (showResetPasswordAction.value) return
    logoClickCount.value += 1
    if (logoClickCount.value >= 5) {
      showResetPasswordAction.value = true
      ElMessage({
        type: 'success',
        message: '已开启密码重置入口'
      })
  ...
  const submitResetPassword = async () => {
    ...
    if (!resetPasswordForm.oldPassword) {
      ElMessage({
        type: 'warning',
        message: '请输入旧密码'
      })
    ...
    if (res.code === 0) {
      ElMessage({
        type: 'success',
        message: '重置成功，请使用新密码登录'
      })
      resetPasswordDialogVisible.value = false
      loginFormData.username = resetPasswordForm.username
      loginFormData.password = resetPasswordForm.newPassword
