  ...
  <div>
    ...
    <div class="gva-table-box">
        <div class="gva-btn-list">
            ...
            <ExportTemplate  template-id="starNote_UserAccount" />
            <ExportExcel  template-id="starNote_UserAccount" filterDeleted/>
            <ImportExcel  template-id="starNote_UserAccount" @on-success="getTableData" />
        </div>
        <el-table
        ref="multipleTable"
        style="width: 100%"
        ...
        <el-table-column sortable align="left" label="日期" prop="CreatedAt" width="180">
            ...
            <el-table-column sortable align="left" label="账号/ID" prop="username" width="120" />

            <el-table-column align="left" label="邮箱或手机" prop="emailPhone" width="120" />

            <el-table-column align="left" label="昵称" prop="nickname" width="120" />

            <el-table-column label="头像路径" prop="avatar" width="200">
    <template #default="scope">
      <el-image preview-teleported style="width: 100px; height: 100px" :src="getUrl(scope.row.avatar)" fit="cover"/>
            ...
            <el-table-column align="left" label="现住址" prop="address" width="120" />
    ...
    {{ detailForm.username }}
                    ...
                    <el-descriptions-item label="加密密码">
    {{ detailForm.password }}
</el-descriptions-item>
                    <el-descriptions-item label="邮箱或手机">
...
const getTableData = async() => {
  const table = await getUserAccountList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteUserAccountByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateUserAccountFunc = async(row) => {
    const res = await findUserAccount({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteUserAccountFunc = async (row) => {
